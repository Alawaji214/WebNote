...
const App = () => {
  ...
  const fetchNotes = async () => {
    ...
    try {
      const response = await fetch('https://note-gateway3-2m0hc1vk.uc.gateway.dev/v1/note/note', {
        method: 'GET',
        headers: {
  ...
  const handleDelete = async (id) => {
    try {
      ...
      const response = await fetch(`https://note-gateway3-2m0hc1vk.uc.gateway.dev/v1/note/note/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`, // Send the token in the Authorization header
        },
