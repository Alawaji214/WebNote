...
const SignupComponent = ({ handleSignIn, setShowSignup }) => {
  ...
  const handleSignup = (event) => {
    ...
    // handle signup
    fetch(`https://note-gateway9-2m0hc1vk.uc.gateway.dev/v1/user/signup`, {
      method: 'POST',
      headers: {
  ...
  const resetPassword = async (email) => {
    try {
      const response = await fetch('https://note-gateway9-2m0hc1vk.uc.gateway.dev/v1/reset/sendPasswordReset', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
